"""
Quantify signal spread and decay from distance heatmap TSV files (v4).

Simplified version focusing on DBSCAN segmentation and clean metrics output.
...
4. Applies DBSCAN clustering to segment signal from noise
5. Calculates simple, robust metrics:
   - Maximum distance reached per timepoint
   - Time to signal disappearance
6. Saves metrics to TSV for downstream analysis
...
def segment_by_clustering(
    ...
    """
    Segment signal using DBSCAN clustering.
    
    Strategy:
    1. Pre-filter pixels above intensity threshold
    2. Cluster based on (distance, time, intensity) in normalized space
    ...
    Args:
        ...
        intensity_threshold_percentile: Percentile for pre-filtering (default: 50.0)
        eps: DBSCAN neighborhood radius (default: 0.20)
        min_samples: Minimum cluster size (default: 30)
        quiet: Suppress logging
    
    Returns:
        Boolean mask marking signal pixels
    """
    def log_info(msg):
        if not quiet:
            logging.info(msg)
    ...
    log_info("=" * 60)
    log_info(f"Parameters: eps={eps}, min_samples={min_samples}, threshold={intensity_threshold_percentile}%ile")
    
    # Pre-filter candidate pixels
    positive_pixels = heatmap_smoothed > 0
    if np.sum(positive_pixels) == 0:
        logging.warning("No positive pixels found")
    ...
    log_info(f"Candidate pixels: {n_candidates} (threshold={threshold:.2f})")
    
    if n_candidates == 0:
        logging.warning("No candidates above threshold")
        return np.zeros_like(heatmap_smoothed, dtype=bool)
...
def process_single_tsv(
    ...
    try:
        ...
        heatmap_min = np.nanmin(heatmap_smoothed)
        heatmap_max = np.nanmax(heatmap_smoothed)
        
        if heatmap_max > heatmap_min:
            heatmap_scaled = ((heatmap_smoothed - heatmap_min) / (heatmap_max - heatmap_min) * 65535).astype(np.uint16)
        else:
            heatmap_scaled = np.zeros_like(heatmap_smoothed, dtype=np.uint16)
        ...
        # DBSCAN segmentation
        signal_mask = segment_by_clustering(
            heatmap_smoothed=heatmap_smoothed,
            distance_bins=distance_bins,
            intensity_threshold_percentile=args.clustering_intensity_threshold,
...
def main():
    parser = argparse.ArgumentParser(
        description='Quantify signal spread and decay from distance heatmap TSV files (v4 - simplified)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Example YAML config for run_pipeline.exe:
---
run:
- name: Quantify with DBSCAN (v4 - clean metrics)
  environment: uv@3.11:segmentation
  commands:
  - python
  - '%REPO%/standard_code/python/plots/quantify_distance_heatmap_v4.py'
...
"""
    ...
    parser.add_argument('--clustering-intensity-threshold', type=float, default=50.0,
                       help='Percentile threshold for clustering (default: 50.0)')
