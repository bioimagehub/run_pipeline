...
## Plots are legacy, not used currently
...
#     # Get monitor size for figure scaling (only when showing plots interactively)
#     if output_path is None:
#         # Interactive mode - try to get screen size
#         try:
#             import tkinter as tk
#             root = tk.Tk()
#             root.withdraw()
...
#             figsize = (6 * n_cols, 5)
#     else:
#         # Saving to file - use fixed reasonable size
#         figsize = (6 * n_cols, 5)
...
#     # Auto-calculate figure size (only when showing plots interactively)
#     if output_path is None:
#         # Interactive mode - try to get screen size
#         try:
#             import tkinter as tk
...
def get_experiment_info_from_filename(filename: str, split: str = "__", colname: list[str] = ["Group"]) -> Dict[str, str]:
    ...
    for i, col in enumerate(colname):
        ...
        if i < len(parts):
            ...
            logging.warning(f"Not enough parts in filename '{filename}' for column '{col}' at index {i}")
            result[col] = None
...
def quantify_signal_spread_over_time(
    image_path: str,
    distance_path: str,
    output_path: Optional[str] = None,
    channel: int = 0,
    ...
    filename_split: str = "__",
    filename_colnames: Optional[List[str]] = None
) -> Optional[Dict]:
    """
    ...
    6. Saves TSV files and TIFF data if output_path is provided
    7. Returns quantitative metrics
    
    Args:
        image_path: Path to the input image file
        distance_path: Path to distance matrix file
        output_path: Optional base path for saving outputs (TSV and TIFF files).
                     If None, only returns results without saving. (default: None)
        channel: Which channel to use from the image (default: 0)
        ...
        filename_split: Delimiter to split filename on (default: "__")
        filename_colnames: List of column names to extract from filename.
                          If None, defaults to ["Experiment", "Group", "Replicate"].
                          Use "_" to skip a part. (default: None)
    ...
    # Save outputs if output_path is provided
    if output_path:
        output_dir = os.path.dirname(output_path)
        if output_dir and not os.path.exists(output_dir):
            os.makedirs(output_dir, exist_ok=True)
        ...
        if filename_colnames is None:
            filename_colnames = ["Experiment", "Group", "Replicate"]
        
        # Extract experimental info from filename
        exp_info = get_experiment_info_from_filename(image_path, split=filename_split, colname=filename_colnames)
        logging.info(f"Experimental info: {exp_info}")
        
        # Build metrics DataFrame dynamically based on extracted columns
        metrics_data = {}
        
        # Add experimental info columns (dynamically from exp_info)
        for colname in filename_colnames:
            if colname != "_":  # Skip placeholder columns
                metrics_data[colname] = [exp_info.get(colname, 'Unknown')] * len(timepoints_arr)
        
        # Add fixed columns
        metrics_data['Filename'] = [Path(image_path).stem] * len(timepoints_arr)
        metrics_data['Timepoint'] = timepoints_arr
        metrics_data['QC_Status'] = ['passed'] * len(timepoints_arr)  # Only QC-passed files are processed
        
        # Add measurement columns
        metrics_data['Center_Distance'] = distance_peak
        metrics_data['Spread_Lower'] = distance_spread_lower
        metrics_data['Spread_Upper'] = distance_spread_upper
        metrics_data['Distance_Spread'] = distance_spread
        metrics_data['Signal_Area_Bins'] = signal_area
        metrics_data['Total_Intensity'] = temporal_intensity
        ...
        # Build decay summary DataFrame dynamically
        decay_data = {}
        
        # Add experimental info columns (dynamically from exp_info)
        for colname in filename_colnames:
            if colname != "_":  # Skip placeholder columns
                decay_data[colname] = [exp_info.get(colname, 'Unknown')]
        
        # Add fixed columns
        decay_data['Filename'] = [Path(image_path).stem]
        decay_data['QC_Status'] = ['passed']  # Only QC-passed files are processed
        
        # Add decay metrics
        decay_data['Signal_Duration_Timepoints'] = [signal_duration]
        decay_data['Time_To_50_Percent'] = [time_to_50_percent]
        decay_data['Time_To_Disappearance'] = [time_to_disappearance]
        decay_data['Decay_Rate_Per_Timepoint'] = [decay_rate]
        decay_data['Initial_Intensity_T1'] = [intensity_T1]
        decay_data['Intensity_T10'] = [intensity_T10]
        decay_data['Intensity_T20'] = [intensity_T20]
...
def process_single_file(
    ...
    try:
        ...
        quantify_signal_spread_over_time(
            img_path,
            dist_path,
            ...
            filename_split=args.filename_split,
...
---
run:
- name: Quantify distance heatmaps with QC filtering (v2)
    environment: uv@3.11:default
    commands:
    - python
    - '%REPO%/standard_code/python/plots/quantify_distance_heatmap_v2.py'
...
- name: Quantify heatmaps with custom smoothing (v2)
    environment: uv@3.11:default
    commands:
    - python
    - '%REPO%/standard_code/python/plots/quantify_distance_heatmap_v2.py'
    ...
    parser.add_argument('--filename-colnames', type=str, nargs='+', default=None,
                       help='Column names to extract from filename split. '
                            ...
                            '(default: ["Experiment", "Group", "Replicate"])')
    ...
    if mode_type == 'group':
        ...
        for basename in basenames:
            ...
            img_path = complete_pairs[basename]['input']
            exp_info = get_experiment_info_from_filename(img_path, split="__", colname=["_", "Group"])
            group = exp_info.get('Group', 'Unknown')
