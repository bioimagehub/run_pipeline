...
def test_drift_correction() -> Tuple[np.ndarray, Dict[str, Any]]:
    """
    ...
    from drift_correction.synthetic_data_generators import  create_simple_squares # hypothetical module for synthetic data
    ...
    test_image_path = os.path.join(tmpdir, "synthetic_test_drift_correction.tif")

    # Save the drifted video as a test image
    rp.save_tczyx_image(drifted_video, test_image_path)
    print(test_image_path)

    shifts, metadata = process_single_file(test_image_path,     
    ...
    if np.allclose(shifts, gt_correction_3d, atol=0.5):  # Allow 0.5 pixel tolerance
        logger.info("✓ DRIFT CORRECTION TEST PASSED: Detected shifts match expected correction shifts!")
        logger.info(f"Perfect accuracy achieved with phase correlation algorithm")
    else:
        logger.warning("✗ DRIFT CORRECTION TEST FAILED: Shifts do not match expected values.")
        mae = np.mean(np.abs(shifts - gt_correction_3d))
        logger.warning(f"Mean absolute error: {mae:.3f} pixels")
        logger.info(f"Detected: {shifts.tolist()}")
        logger.info(f"Expected: {gt_correction_3d.tolist()}")
...
if __name__ == "__main__":
    test_drift_correction()  # Run as script with argument parsing
