...
def convert_single_file(
    ...
    try:
        ...
        if len(scenes) > 1:
            ...
            # For obf files from Abberior STED We can have many pyramid levels with different resolutions so the max will not work
            if input_path.lower().endswith(".obf"):
                # Handle OBF-specific logic here
                # My idea is that all the low res images have the smallest and the same dimensions, so we can find the smallest and process larger
                min_pixels = min(dims[0] * dims[1] for dims in scene_dims.values()) + 1
            else: # All other formats           
                # Find the largest dimension (assuming this is the full resolution)
                min_pixels = max(dims[0] * dims[1] for dims in scene_dims.values())
            
            # Only keep scenes with more pixels than the minimum 
            for scene_id, dims in scene_dims.items():
                if dims[0] * dims[1] >= min_pixels:
