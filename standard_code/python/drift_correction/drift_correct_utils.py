...
def gaussian_blur_2d_gpu(data: np.ndarray, sigma: float = 1.0, channel: Optional[int] = None) -> np.ndarray:
    """
    Apply 2D Gaussian blur to all YX planes of a TCZYX image using CuPy GPU acceleration.
    
    Args:
        data: Input numpy array of shape TCZYX
        sigma: Standard deviation for Gaussian kernel. Higher values = more blur.
        channel: If specified, only apply blur to this channel index. If None, apply to all channels.
    
    Returns:
        Blurred numpy array of same shape as input
        
    Raises:
        ValueError: If input array is not 5D TCZYX format
        ImportError: If CuPy is not available and scipy fallback fails
    """
    if data.ndim != 5:
        raise ValueError(f"Expected 5D TCZYX array, got {data.ndim}D with shape {data.shape}")
    ...
    try:
        ...
        #logger.info(f"Using CuPy GPU acceleration for Gaussian blur (sigma={sigma})")
        
        # Copy to GPU memory
        data_gpu = cp.asarray(data, dtype=cp.float32)
        blurred_gpu = cp.zeros_like(data_gpu)
        
        # Determine which channels to process
        channels_to_process = [channel] if channel is not None else list(range(C))
        
        # Copy original data first
        blurred_gpu[:] = data_gpu
        
        # Apply 2D blur to specified channels only
        for t in range(T):
            for c in channels_to_process:
                for z in range(Z):
                    # Apply 2D Gaussian filter to YX plane
                    blurred_gpu[t, c, z, :, :] = gaussian_filter(
                        data_gpu[t, c, z, :, :], 
                        sigma=sigma, 
        ...
        logger.warning("CuPy not available, falling back to CPU with scipy")
        
        # Fallback to scipy CPU implementation  
        try:
            ...
            for t in range(T):
                for c in channels_to_process:
                    for z in range(Z):
                        result[t, c, z, :, :] = gaussian_filter(
                            data[t, c, z, :, :].astype(np.float32),
                            sigma=sigma,
                            mode='reflect'
                        )
            
            logger.info(f"Applied Gaussian blur using scipy CPU (sigma={sigma})")
            return result
            
        except ImportError:
            raise ImportError("Neither CuPy nor scipy available for Gaussian filtering")
