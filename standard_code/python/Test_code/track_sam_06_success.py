...
sam_tile_hw       = 256    # SAM2 spatial tile size in px; set None to disable tiling (full frame)
...
for t_idx in range(n_total):
    for blob_y, blob_x, sigma in blobs_per_frame[t_idx]:
        ...
        # Crop a tile around the blob — fill-holes must run on a local crop
        # so that the border-seeded reconstruction works correctly for holes
        # surrounded by bright membrane deep inside the frame.
        y0 = max(0, blob_y - tile_size)
        y1 = min(H, blob_y + tile_size)
        x0 = max(0, blob_x - tile_size)
        x1 = min(W, blob_x + tile_size)
        ...
        tile = raw_frames[t_idx][y0:y1, x0:x1].astype(np.float32)
        ...
        if perim > 0 and (4.0 * np.pi * area / perim ** 2) < min_circularity:
            continue
        # Write back to global coords (no-overwrite)
        write_region = raw_mask & (det_mask_tyx[t_idx, y0:y1, x0:x1] == 0)
        det_mask_tyx[t_idx, y0:y1, x0:x1][write_region] = 1
...
# %% save as tif
output_path = os.path.join(output_folder, "global_mask_v6.tif")
import tifffile
