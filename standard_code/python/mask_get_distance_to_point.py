...
def process_folder(args: argparse.Namespace, parallel:bool) -> None:
    yaml_files = rp.get_files_to_process(args.input_folder, args.input_yaml_extension, search_subfolders=False)
    os.makedirs(args.output_folder, exist_ok=True)

    stem_len = len(args.input_yaml_extension)
    base_names = [f[:-stem_len] if f.endswith(args.input_yaml_extension) else f for f in yaml_files]
    mask_files = [f + args.input_mask_extension for f in base_names]

    tasks = list(zip(yaml_files, mask_files))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input-folder", type=str, required=True, help="Folder containing input YAML and mask files.")
    parser.add_argument("--input-yaml-extension", type=str, default="_metadata.yaml", help="YAML file extension.")
    parser.add_argument("--input-mask-extension", type=str, default=".tif", help="Mask image extension.")
